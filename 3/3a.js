...
fs.readFile("./3/3a.txt", "utf8", (err, data) => {
  ...
  for (let i = 0; i < dataSplit.length; i++) {
    const element = dataSplit[i];
    for (let j = 0; j < element.length; j++) {
      const char = element[j];
      reversed[j].push(char);
    }
  }
  ...
  reversed.forEach((element) => {
    let ones = element.filter((i) => {
      return i == 1;
    });
    let zeroes = element.filter((i) => {
      return i == 0;
    });
  ...
  console.log("Gamma: " + parseInt(gammaRateBinary, 2));
  console.log("Epsilon: " + parseInt(EpsilonRateBinary, 2));
  console.log(
    "PowerConsumption: " +
